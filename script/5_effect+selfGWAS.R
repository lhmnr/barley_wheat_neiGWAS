...
sigSNPs_effect_disc = ggplot(sig_SNPs_effect, aes(x=distance,y=b,group=SNP_info,colour=SNP_info)) + geom_line() + geom_hline(yintercept = 0) + ggtitle("effects of base difference in sig. SNPs on susceptibility for each distance") + scale_x_discrete(limits = x_axis_order) + xlab("distance of included neighbours") + ylab("effect on susceptibilty (1x for different base, 0,5x for heterozygous)")
sigSNPs_p_disc = ggplot(sig_SNPs_p, aes(x=distance,y=p_log10,group=SNP_info,colour=SNP_info)) + geom_line() + geom_hline(yintercept = -log10(0.05/nrow(gwas_results))) + ggtitle("p-values of effects of base difference in sig. SNPs on susceptibility for each distance") + scale_x_discrete(limits = x_axis_order) + xlab("distance of included neighbours (0 means self-GWAS)") + ylab("-log10 p-value")
...
sigSNPs_effect_cont = ggplot(sig_SNPs_effect, aes(x=distance,y=b,group=SNP_info,colour=SNP_info)) + geom_line() + geom_hline(yintercept = 0) + ggtitle("effects of base difference in sig. SNPs on susceptibility for each distance") + xlab("distance of included neighbours") + ylab("effect on susceptibilty (1x for different base, 0,5x for heterozygous)")
sigSNPs_p_cont = ggplot(sig_SNPs_p, aes(x=distance,y=p_log10,group=SNP_info,colour=SNP_info)) + geom_line() + geom_hline(yintercept = -log10(0.05/nrow(gwas_results))) + ggtitle("p-values of effects of base difference in sig. SNPs on susceptibility for each distance") + xlab("distance of included neighbours (0 means self-GWAS)") + ylab("-log10 p-value")
