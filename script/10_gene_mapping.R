...
daynumber_LD = "20231026"
#daynumber of 9_gene_analysis output
daynumber_genes = "20240304"
...
for(j in 1:length(SNP_names)){
  for(i in 1:length(SNP_names)){
    rect(inter_SNP_pos[i], j-0.8, inter_SNP_pos[i+1], j-0.2, col = rgb((v_LD[i,j]), 0.0, 0.0), lwd = 0)
    rect(inter_SNP_pos[i], j-0.2, inter_SNP_pos[i+1], j-0.0, col = rgb(0.0, logp_scaled[i], 0.0), lwd = 0)
  }
  ...
  if(SNP_sigtype[j]==1){
    ...
    polygon(poly_x,poly_y,col="white",border=FALSE)
  }
...
mtext(paste0("phenotype: ",poolName,", range: ",chromosome,":",1000*SNP_pos[1],"-",chromosome,":",1000*SNP_pos[length(SNP_pos)],", SNPs: ",length(SNP_pos),", max -log(p): ",maxlogp,", transcripts: ",nrow(transcripts)), col = "white")

par(mar = c(2.0,4.3,0.0,9.1))
plot(c(pos_min, pos_max), c(0.0, maxlogp), type = "n", xlab = "", ylab = "-log(p_nei) (white), scaled LD (red) and -log(p_self) (green)", yaxt = "n") + axis(side = 2, col = "white") + axis(side = 1, col = "white") + axis(side = 4, at = sig_levels,labels = sig_names, las = 2)
for(k in 1:nrow(transcripts)){
...
for(k in 1:nrow(transcripts)){
  rect(transcripts$left[k], 2.0 + transcripts_offset[k], transcripts$right[k], 3.0 + transcripts_offset[k], col = rgb(0.2, 0.2, 1.0, 0.5), border = "white")
}
for(j in 1:length(SNP_names)){
  poly_x = c(SNP_pos[j]-(0.5*symbol_radius_x),SNP_pos[j],SNP_pos[j]+(0.5*symbol_radius_x))
  poly_y = c(0.5,1.5,0.5)
  polygon(poly_x, poly_y, col = rgb(0.0, logp_scaled[j], 0.0), border="white")
  for(i in 1:length(sig_SNPs)){
    poly_y = c(-i,1-i,-i)
    polygon(poly_x, poly_y, col = rgb((v_LD[j,sig_SNPs[1+length(sig_SNPs)-i]]), 0.0, 0.0), border="white")
  }
}
text(x = SNP_pos, y = par("usr")[3] - 0.25, srt = -90, adj = 0, labels = SNP_names, xpd = TRUE, col = "white")
...
for(j in 1:4){
  ...
  plot(c(pos_min + (pos_dif*(j-1)/4), pos_min + (pos_dif*j/4)), c(0.0, 2.0), type = "n", xlab = "", ylab = "candidate transcripts (blue)", yaxt = "n") + axis(side = 1, col = "white", at = c(floor((pos_min + (pos_dif*(j-1)/4))/100):ceiling((pos_min + (pos_dif*j/4))/100))*100, xpd = TRUE) + axis(side = 3, at = (transcripts$left+transcripts$right)/2, labels = transcripts$name, las = 2, mgp = c(3.0,0.0,0.0))
  for(k in 1:nrow(transcripts)){
    rect(transcripts$left[k], transcripts_offset[k], transcripts$right[k], 1.0 + transcripts_offset[k], col = rgb(0.2, 0.2, 1.0, 0.5), border = "white")
  }
  par(mar = c(9.1,4.3,0.1,9.1))
  plot(c(pos_min + (pos_dif*(j-1)/4), pos_min + (pos_dif*j/4)), c(0.0, maxlogp), type = "n", xlab = "", ylab = "-log(p_nei) (white), scaled LD (red) and -log(p_self) (green)", yaxt = "n", xaxt = "n") + axis(side = 2, col = "white") + axis(side = 4, at = sig_levels,labels = sig_names, las = 2)
  for(i in 0:ceiling(1.2*maxlogp)){
    abline(h = i, col = rgb(0.2,0.2,0.2))
  }
  for(k in 1:nrow(transcripts)){
    rect(transcripts$left[k], 0.0, transcripts$right[k], maxlogp, col = rgb(0.2, 0.2, 1.0, 0.5), lwd = 0)
  }
