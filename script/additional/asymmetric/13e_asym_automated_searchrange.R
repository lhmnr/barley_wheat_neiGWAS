...
for(i in 1:nrow(locus_info)){
  ...
  bgc <- par(bg = "black")
  par(col.main = "white",col.axis = "white", col.lab = "white")
  plot(c(pos_min, pos_max), c(0.0, 1.1*max(gwas_results_i[,c(3,4,5)])), type = "n", xlab = paste0("position on chromosome ",sigSNPs_i[1,2]), ylab = "-log(p) (self in yellow, nei in turquoise, sxn in pink)", main = paste0("-log(p)-values in automatically genereated search range"), yaxt = "n", xaxt = "n")
  axis(side = 1, col = "white")
  axis(side = 2, col = "white")
  for(k in 1:nrow(subranges_i)){
    rect(subranges_i$pos_l[k], 0.0, subranges_i$pos_r[k], 1.1*max(gwas_results_i[,c(3,4,5)]), col = rgb(0.1, 0.1, 0.1), border = rgb(0.5, 0.5, 0.5))
  }
  abline(h = -log10(0.05/nSNPs), col = rgb(1.0,1.0,1.0))
  lines(x = gwas_results_i[,2], y = gwas_results_i[,3], col = rgb(1.0,1.0,0.0))
  lines(x = gwas_results_i[,2], y = gwas_results_i[,4], col = rgb(0.0,1.0,1.0))
  lines(x = gwas_results_i[,2], y = gwas_results_i[,5], col = rgb(1.0,0.0,1.0))
  mtext(paste0("phenotype: ",sigSNPs_i[1,1],", range: ",sigSNPs_i[1,2],":",pos_min,"-",pos_max,", type: ",sigSNPs_i[1,4],", SNPs: ",nrow(sigSNPs_i)), col = "white")
  dev.off()
}
